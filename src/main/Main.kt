            ...
            try {
                ...
                val personal: Personal = if (data[7] == "Entrenador") {
                    Entrenador(
                        ...
                        data[5].toDouble(),
                        data[6],
                        Entrenador.Especializacion.valueOf(data[8]).toString()
                    )
                } else {
                    Jugador(
                        id,
                        ...
                        data[5].toDouble(),
                        data[6],
                        Jugador.Posicion.valueOf(data[9]).toString(),
                        data[10].toInt(),
                        data[11].toDouble(),
                        data[12].toDouble(),
                        data[13].toInt(),
...
fun crearMiembro() {
    ...
    when (readln()) {
        "E" -> {
            ...
            val id = generarId()
            val entrenador = Entrenador(id, nombre, apellidos, fechaNacimiento, fechaIncorporacion, salario, pais)
            personalCache.put(id, entrenador)
        }
        "J" -> {
            ...
            val id = generarId()
            val jugador = Jugador(id, nombre, apellidos, fechaNacimiento, fechaIncorporacion, salario, pais,
                posicion.toString(), dorsal, altura, peso, goles, partidosJugados)
            personalCache.put(id, jugador)
        }
        else -> println("Opción no válida o inexistente.")
    }
...
fun actualizarMiembro() {
    ...
    if (miembro != null) {
        ...
        if (miembro is Entrenador) {
            ...
            miembro.especialidad = Entrenador.Especializacion.valueOf(readln()).toString()
        } else if (miembro is Jugador) {
            print("Nueva posición (DEFENSA, CENTROCAMPISTA, DELANTERO, PORTERO): ")
            miembro.posicion = Jugador.Posicion.valueOf(readln()).toString()
            print("Nuevo dorsal: ")
...
fun guardarDatosEnCsv(fileName: String) {
    ...
    file.printWriter().use { out ->
        personalCache.listAll().forEach { personal ->
            when (personal) {
                is Entrenador -> out.println("${personal.id},${personal.nombre},${personal.apellidos},${personal.fechaNacimiento},${personal.fechaIncorporacion},${personal.salario},${personal.pais},Entrenador,${personal.especialidad
                },,,,")
...
fun consultaDelanteroMasAlto() {
    val delanteroMasAlto = personalCache.listAll().filterIsInstance<Jugador>().filter { it.posicion == Jugador.Posicion.DELANTERO.toString() }.maxByOrNull { it.altura }
    println("El delantero más alto es: $delanteroMasAlto")
}

fun consultaMediaGolesDelanteros() {
    val delanteros = personalCache.listAll().filterIsInstance<Jugador>().filter { it.posicion == Jugador.Posicion.DELANTERO.toString() }
...
fun consultaDefensaMasPartidos() {
    val defensaMasPartidos = personalCache.listAll().filterIsInstance<Jugador>().filter { it.posicion == Jugador.Posicion.DEFENSA.toString() }.maxByOrNull { it.partidosJugados }
